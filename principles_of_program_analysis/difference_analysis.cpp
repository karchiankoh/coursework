/*
PROJECT DESCRIPTION:
The difference analysis serves to determine, for each program point, and for each pair of 
variables, the largest separation. In this task, implement an LLVM pass to perform the 
difference analysis. Note: Your analysis can be limited to the following instructions: 
Alloca, Add, Sub, Div, Mul, Rem and Load/Store. In order to guarantee termination, the 
analysis may now not be able to produce a finite bound at all times. You therefore need to 
...
std::set<int> abstractDomain = { NEG_INF, -20, -10, -1, 0, 1, 10, 20, POS_INF};
...
BBANALYSIS applyCond(BBANALYSIS predSet, BasicBlock* predecessor, BasicBlock* BB)
{
	for (auto &I: *predecessor)
	{
		if (isa<BranchInst>(I)){
			...
			std::pair<int, int> set1, set2;
			set1 = retrieveSet(op1, predSet);
			set2 = retrieveSet(op2, predSet);
			...
			if(isa<LoadInst>(dyn_cast<Instruction>(cmp->getOperand(0)))){
				std::pair<int, int> tempSet;
				Instruction* l =  dyn_cast<Instruction>(cmp->getOperand(0));
				switch(cmpValue) {
					case 0: {
						tempSet.first = NEG_INF;
						tempSet.second = set2.second;
						break;
					}
					case 1: {
						tempSet.first = set2.first + 1;
				    	tempSet.second = POS_INF;
						break;
					}
					case 2: {
						tempSet.first = NEG_INF;
						tempSet.second = set2.second - 1;
						break;
					}
					case 3: {
						tempSet.first = set2.first;
				    	tempSet.second = POS_INF;
						break;
					}
					case 4: {
    ...
    while(!fixPointReached(oldAnalysisMap)){
		...
		for ( auto it = abstractAnalysisMap.begin();it != abstractAnalysisMap.end(); ++it)
		{
			...
			for ( auto it1 = analysis.begin();it1 != analysis.end(); ++it1){
				...
				bool hasPastSelf = false;
				for ( auto it2 = analysis.begin();it2 != analysis.end(); ++it2){
					if (it1->first == it2->first) {
						hasPastSelf = true;
					} else if (hasPastSelf) {
	    				std::pair<int, int> set2 = it2->second;
	    				llvm::errs() << "\t";
		    			it1->first->dump();
		    			llvm::errs() << "\t";
		    			it2->first->dump();
		    			int sep1 = abs(subtract(set1.second, set2.first));
		    			int sep2 = abs(subtract(set2.second, set1.first));
		    			llvm::errs() << "\t\t\t\t\tSep: ";
	    				if(sep1 >= sep2) llvm::errs() << sep1;
	    				else llvm::errs() << sep2;
		    			llvm::errs() << "\n";
		    		}
	    		}
